...
def generate_from_model(model, g, length, batch_size):
    ...
    for t in range(length-1):
        ...
        row_v = eye[[np.random.choice(nwords, p=p) for p in pvec]]
        rows.append(row_v)
...
def generate_batch(g, length, batch_size):
    ...
    sequences = np.array([g.generate(length+1, as_vectors=True)[:length+1] for _ in range(batch_size)])
...
def test(model, g, length, batch_size, samples):
    ...
    while nsamples < samples:
        ...
        generated = [g.vectors_to_words(seq) for seq in generated]
...
if __name__ == '__main__':
    ...
    opts, args = getopt.getopt(sys.argv[1:], "b:l:d:w:")
    opts = dict(opts)
    ...
    g = Generator(nwords)
    model = create_net(g.AlphabetSize)
